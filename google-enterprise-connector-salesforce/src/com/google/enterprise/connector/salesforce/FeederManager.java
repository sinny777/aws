 ...
 * <p>
 * This class holds a hashmap with ALL the {@link BaseTraversalManager}
 * BaseTraversalManager inside this VM. THe quartz callback will pass in the
 * connector name thats in context and we use that name to lookup the
 * BaseTraversalManager.
 * </p>
 */
public class FeederManager {
	...
	/**
	 * Registers the traversal manager and connector instances with this
	 * singleton so that we have a handle to the connector when quartz invokes
	 * it.
	 * 
	 * @param btm
	 *            the traversal manager to register
	 * @param connector
	 *            the connector in context
	 */

	public void setTraversalCallback(BaseTraversalManager btm,
			BaseConnector connector) {
		try {
			logger.log(
					Level.INFO,
					">>>>>>>>>>>>>>>> FeederManager setConnector "
			...
			// the naming convention used is the scheduler is called
			// 'connectorinstancenameScheduler'
			sched = schedFact.getScheduler(connector.getInstanceName()
					+ "Scheduler");

			// if this is the first time initializeing, create a new scheduler
			// from scratch
			if (sched == null) {
				...
				if (connector.getStoretype().equalsIgnoreCase("DBStore")) {
					// first check if its a dbstore, then we're storing hte
					// quartz info in
					...
					JNDIConnectionProvider jcp = new JNDIConnectionProvider(
							"java:comp/env/"
									+ BaseConstants.CONNECTOR_DATASOURCE, true);
					dcm.addConnectionProvider("java:comp/env/"
							+ BaseConstants.CONNECTOR_DATASOURCE, jcp);
					JobStoreCMT jsCMT = new JobStoreCMT();
					// todo: the driverdelegate works for mysql+tomcat...others
					// may
					...
					jsCMT.setNonManagedTXDataSource("java:comp/env/"
							+ BaseConstants.CONNECTOR_DATASOURCE);
					jsCMT.setDataSource("java:comp/env/"
							+ BaseConstants.CONNECTOR_DATASOURCE);
					jsCMT.setTablePrefix("QRTZ_");
					jsCMT.setIsClustered(true);
					jsCMT.setInstanceId(StdSchedulerFactory.AUTO_GENERATE_INSTANCE_ID);
					jsCMT.setInstanceName(connector.getInstanceName()
							+ "Instance");
				...
				DirectSchedulerFactory.getInstance().createScheduler(
						connector.getInstanceName() + "Scheduler",
						connector.getInstanceName() + "Instance", threadPool,
						jobStore);
			}
			...
			sched = schedFact.getScheduler(connector.getInstanceName()
					+ "Scheduler");
			JobDetail jobDetail = new JobDetail(connector.getInstanceName(),
					connector.getInstanceName(), ScheduledJob.class);
			jobDetail.getJobDataMap().put("type", "FULL");
			// just do failover
			jobDetail.setRequestsRecovery(true);
			CronTrigger trigger = new CronTrigger(connector.getInstanceName(),
					connector.getInstanceName());
			logger.log(Level.INFO,
					"ATTEMPTING TO SET SCHEDULE  [" + connector.getSchedule()
							+ "] for " + connector.getInstanceName());
			trigger.setCronExpression(connector.getSchedule());
			sched.deleteJob(connector.getInstanceName(),
					connector.getInstanceName());
	...
	/**
	 * When quartz fires, it calls this class in the singelton with the name of
	 * the connector that just got fired
	 * 
	 * @param connector_name
	 *            the connector whose quartz just got triggered
	 */
	...
	public void QuartzCallback(String connector_name) {
		...
		logger.log(Level.INFO, "QuartzCallback called for: [" + connector_name
				+ "]");
		BaseTraversalManager btm = (BaseTraversalManager) hm_sf_traversal
				.get(connector_name);
		...
		if (btm.getConnector().IsRunning()) {
			logger.log(Level.INFO,
					"Previous quartz run is active. Aborting current run  ["
							+ connector_name + "]");
			return;
		}
		...
		if (sfm.login()) {
			...
			System.setProperty(btm.getConnector().getInstanceName()
					+ "_last_crawl_date", last_crawled_date);
