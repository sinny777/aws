...
public class SalesForceModule {
	 ...
	 * 
	 * @param btm
	 *            BaseTraversalManager storing the username/session info
	 */
	...
	public boolean login() {
		try {
			sfl = new SalesForceLogin(connector.getUsername(),
					connector.getPassword(), connector.getLoginsrv());
	 ...
	 * 
	 * @param store
	 *            IStoreType the store type to populate
	 */
	public void populateStore(IStoreType store) {
		...
		String last_sync_date = Util
				.convertCheckPoint_Date_to_SF_Date_(ckpt_dt);
		String modified_sf_query = query.replace("$LAST_SYNC_DATE",
				last_sync_date);
		...
		try {
			...
			modified_sf_query = java.net.URLDecoder.decode(modified_sf_query,
					"UTF-8");

			// get ready to query
			SFQuery sfq = new SFQuery();
			// submit the query and get the XML Document response
			Document soap_response_doc = sfq.submitStatement(modified_sf_query,
					BaseConstants.QUERY_TYPE, sfl.getEndPointServer(),
					sfl.getSessionID());
			...
			while (q_locator != null) {
				logger.log(Level.INFO, "Query locator cursor found "
						+ q_locator);
				soap_response_doc = sfq.submitStatement(q_locator,
						BaseConstants.QUERYMORE_TYPE, sfl.getEndPointServer(),
						sfl.getSessionID());
			...
			logger.log(
					Level.SEVERE,
					"SFException  to continue Processing Records "
							+ e.getMessage());
		} catch (Exception e) {
			logger.log(
					Level.SEVERE,
					"Exception  to continue Processing Records "
							+ e.getMessage());
		}
	 ...
	 *            IStoreType the store type to populate
	 * @param crawl_date
	 *            the time Salesforce was queried
	 * @param docXML
	 *            the raw SOAP response from the query
	 */
	private void pushBatch(IStoreType store, Date crawl_date, Document docXML) {
		try {
			...
			ByteArrayInputStream bis = new ByteArrayInputStream(sb1.toString()
					.getBytes("UTF-8"));
